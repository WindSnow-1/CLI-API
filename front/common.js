...
function createCredCard(credInfo, manager) {
    ...
    div.innerHTML = `
        ...
        <div class="cred-quota-details" id="quota-${pathId}">
            <div class="cred-quota-content" data-filename="${filename}" data-loaded="false">
                <div style="text-align:center;padding:12px;color:var(--text-secondary);font-size:12px;">加载额度中...</div>
            </div>
        </div>
        ` : ''}
    `;
    ...
    // Antigravity 凭证自动加载额度
    if (managerType === 'antigravity') {
        setTimeout(() => {
            const quotaEl = document.getElementById('quota-' + pathId);
            if (quotaEl) {
                const contentDiv = quotaEl.querySelector('.cred-quota-content');
                if (contentDiv) loadQuotaData(pathId, filename, contentDiv);
            }
        }, 100);
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            requestAnimationFrame(() => {
                if (typeof initTabSlider === 'function') initTabSlider();
...
async function toggleAntigravityQuotaDetails(pathId) {
    ...
    if (isShowing) {
        ...
        // 每次展开都重新加载数据
        if (filename) {
            await loadQuotaData(pathId, filename, contentDiv);
        }
    }
}
...
const PRIORITY_MODELS = [
    'claude-opus-4-6', 'claude-sonnet-4-6', 'claude-sonnet-4-5', 'claude-opus-4-6-thinking',
...
function getQuotaPriority(name) {
    const lower = name.toLowerCase();
    for (let i = 0; i < PRIORITY_MODELS.length; i++) {
        if (lower.includes(PRIORITY_MODELS[i])) return i;
    }
    if (lower.includes('claude')) return PRIORITY_MODELS.length;
    if (lower.includes('gemini-3')) return PRIORITY_MODELS.length + 1;
    if (lower.includes('gemini-2')) return PRIORITY_MODELS.length + 2;
    return PRIORITY_MODELS.length + 100;
}

async function loadQuotaData(pathId, filename, contentDiv) {
    // 有缓存直接渲染，不重新请求
    if (quotaCache[pathId]) {
        renderQuotaPage(pathId, contentDiv, 1);
        return;
    }

    contentDiv.innerHTML = '<div style="text-align:center;padding:12px;color:var(--text-secondary);font-size:12px;">加载额度中...</div>';
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            if (Object.keys(models).length === 0) {
                contentDiv.innerHTML = '<div style="text-align:center;padding:24px;color:var(--text-secondary);">暂无额度信息</div>';
                return;
            }

            quotaCache[pathId] = { filename, models };
            renderQuotaPage(pathId, contentDiv, 1);
        } else {
            const errorMsg = data.error || '获取额度信息失败';
            contentDiv.innerHTML = `<div style="text-align:center;padding:20px;color:var(--red);">获取额度信息失败: ${errorMsg}</div>`;
        }
    } catch (error) {
        contentDiv.innerHTML = `<div style="text-align:center;padding:20px;color:var(--red);">网络错误: ${error.message}</div>`;
    }
}

function renderQuotaPage(pathId, contentDiv, page) {
    const cache = quotaCache[pathId];
    if (!cache) return;

    const { filename, models } = cache;
    const entries = Object.entries(models);
    const sorted = entries.sort((a, b) => getQuotaPriority(a[0]) - getQuotaPriority(b[0]));

    const totalPages = Math.ceil(sorted.length / QUOTA_PAGE_SIZE);
    if (page < 1) page = 1;
    if (page > totalPages) page = totalPages;

    const start = (page - 1) * QUOTA_PAGE_SIZE;
    const pageItems = sorted.slice(start, start + QUOTA_PAGE_SIZE);

    const full = entries.filter(([,d]) => (d.remaining || 0) >= 1).length;
    const partial = entries.filter(([,d]) => { const r = d.remaining || 0; return r > 0 && r < 1; }).length;
    const empty = entries.filter(([,d]) => (d.remaining || 0) <= 0).length;

    let html = `<div class="quota-panel">
        <div class="quota-summary">
            <span class="quota-summary-item"><span class="quota-dot green"></span>${full} 满额</span>
            <span class="quota-summary-item"><span class="quota-dot orange"></span>${partial} 部分</span>
            <span class="quota-summary-item"><span class="quota-dot red"></span>${empty} 耗尽</span>
            <span class="quota-summary-total">${entries.length} 个模型</span>
            <button class="quota-refresh-btn" onclick="refreshQuota('${pathId}')" title="刷新额度">&#x21bb;</button>
        </div>
        <div class="quota-list">`;

    for (const [modelName, quotaData] of pageItems) {
        const remaining = quotaData.remaining || 0;
        const pct = Math.round(remaining * 100);
        const resetTime = quotaData.resetTime || '';

        let statusClass = 'green';
        if (pct <= 0) statusClass = 'red';
        else if (pct <= 30) statusClass = 'orange';
        else if (pct <= 60) statusClass = 'yellow';

        html += `<div class="quota-row">
            <span class="quota-row-name" title="${modelName}">${modelName}</span>
            <div class="quota-row-bar"><div class="quota-row-bar-fill ${statusClass}" style="width:${pct}%"></div></div>
            <span class="quota-row-pct ${statusClass}">${pct}%</span>
            ${resetTime ? `<span class="quota-row-reset">${resetTime}</span>` : '<span class="quota-row-reset"></span>'}
        </div>`;
    }
    ...
    if (totalPages > 1) {
        html += `<div class="quota-pager">
            <button class="quota-pager-btn" ${page <= 1 ? 'disabled' : ''} onclick="quotaGoPage('${pathId}', ${page - 1})">&#x2039;</button>
            <span class="quota-pager-info">${page} / ${totalPages}</span>
            <button class="quota-pager-btn" ${page >= totalPages ? 'disabled' : ''} onclick="quotaGoPage('${pathId}', ${page + 1})">&#x203a;</button>
        </div>`;
    }
...
function quotaGoPage(pathId, page) {
    const quotaDetails = document.getElementById('quota-' + pathId);
    if (!quotaDetails) return;
    const contentDiv = quotaDetails.querySelector('.cred-quota-content');
    if (!contentDiv) return;
    renderQuotaPage(pathId, contentDiv, page);
}

async function refreshQuota(pathId) {
    const cache = quotaCache[pathId];
    if (!cache) return;
    const quotaDetails = document.getElementById('quota-' + pathId);
    if (!quotaDetails) return;
    const contentDiv = quotaDetails.querySelector('.cred-quota-content');
    if (!contentDiv) return;
    delete quotaCache[pathId];
    await loadQuotaData(pathId, cache.filename, contentDiv);
}

async function refreshAllQuotas() {
    const keys = Object.keys(quotaCache);
    if (keys.length === 0) {
        showStatus('没有已加载的额度数据，请先打开凭证管理页面', 'info');
        return;
    }
    showStatus(`正在刷新 ${keys.length} 个凭证的额度...`, 'info');
    ...
    for (const pathId of keys) {
        ...
        if (!cache) continue;
        const quotaDetails = document.getElementById('quota-' + pathId);
        if (!quotaDetails) continue;
        const contentDiv = quotaDetails.querySelector('.cred-quota-content');
        if (!contentDiv) continue;
        const filename = cache.filename;
        delete quotaCache[pathId];
        tasks.push(loadQuotaData(pathId, filename, contentDiv));
    }

    await Promise.all(tasks);
    showStatus(`已刷新 ${tasks.length} 个凭证的额度`, 'success');
}
...
async function fetchAndDisplayVersion() {
    try {
        ...
        if (data.success) {
            ...
            if (versionDisplay) versionDisplay.textContent = '未知版本';
        }
    } catch (error) {
        console.error('获取版本信息失败:', error);
        const versionText = document.getElementById('versionText');
        if (versionText) versionText.textContent = '版本信息获取失败';
        const versionDisplay = document.getElementById('versionDisplay');
        if (versionDisplay) versionDisplay.textContent = '版本信息获取失败';
    }
}
